...
def train(
    ...
    out_dir: str = "./gpt2_delta_scrub_silu_L2-4",
):
    ...
    while step < steps:
        for batch in dl:
            ...
            conf = 0.0
            last_mse = torch.tensor(0.0, device=device)
            ...
            if k < 0:
                ...
                last_mse = F.mse_loss(vpos, vneg).detach()
            for k in scrub_layers:
                hpos_k = out_pos.hidden_states[k]  # (B,T,D) note: hidden_states[0] is embeddings
                hneg_k = out_neg.hidden_states[k]
                vpos = select_last_hidden(hpos_k, pos_mask)  # (B,D)
                vneg = select_last_hidden(hneg_k, neg_mask)
                conf = conf + layer_w[k] * F.leaky_relu(F.mse_loss(vpos, vneg)-last_mse)
                last_mse = F.mse_loss(vpos, vneg).detach()
